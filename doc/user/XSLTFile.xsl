	...
	<html>
		<body>
			...
			<table width="3000" border="1">
				...
				<xsl:if test="TestcaseResult = 'FAIL' or TestcaseResult = 'ERROR_CODES_DIFFER'">
				<tr>
				<td><xsl:value-of select="TestID"/></td>
				<xsl:choose>
				<xsl:when test="TestcaseFailureReason = 'Expected error code does not match with actual error code.'">
				<td bgcolor="Orange"><xsl:value-of select="TestcaseResult"/></td>
				</xsl:when>
				...
				<xsl:when test="TestcaseFailureReason = 'Test case execution failed. Check log file.'">
