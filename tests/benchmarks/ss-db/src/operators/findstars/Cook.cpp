...
{
  //while(_provider.hasNextArray())
  //{
  _initialize();
  //cout << "Cooker init" << endl;
  _provider.onInitialize();
  //cout<< "Cooking of: " << _provider.getId() << endl; flush(cout);
  PixVal pix;
  for (bool remaining = _provider.moveToFirstPixel(); remaining; remaining = _provider.moveToNextPixel()) 
  {
     //cout<< "Trying to set position " << pix.coord.first << " " <<pix.coord.second ; flush(cout);
     _provider.getCurrentPixel(pix);
     //cout << " .. Done" << endl; flush(cout);
     _checkNext(pix.coord.first, pix.coord.second);
     //cout << " .. Done checking" << endl; flush(cout);
     _ingest(pix);
     //cout << " .. Done ingesting" << endl; flush(cout);
  }
  //cout << "done with " << imageId << endl; flush(cout);
  pix.val = 0;
  pix.coord.second++;
  for (int i = _width-2; i < _width; i++) {
      pix.coord.first = i;
      _checkNext(pix.coord.first, pix.coord.second);
      _ingest(pix);
  }
  //cout << "extra processing " << imageId << endl; flush(cout);
  _flushFinalizable();  // Post-process and write out.
  //cout << "flushed " << imageId << endl; flush(cout);
  //_provider.nextArray();
  //cout << "finalized " << imageId << endl; flush(cout);
  //}
}
...
void Cook::dumpStars() 
{
    ...
    //     array[0] = new int[_height*_width];
    std::vector< std::vector<int> > array(_height, vector<int>(_width));

    for(map<Oid,Observ>::iterator obsItr = _obs.begin(); obsItr != _obs.end(); ++obsItr) {
        Observ obs = obsItr->second;
        PixVect pixels = obsItr->second.pixels;
        for(vector<PixVal>::iterator pixel = pixels.begin(); pixel != pixels.end(); ++pixel) {
//             cout << pixel->coord.first << " " << pixel->coord.second << "(" << obsItr->first << ") ";
//             flush(cout);
            array[pixel->coord.first][pixel->coord.second] = obsItr->first;
        }
//         cout << endl;
//         flush(cout);
    }
...
void Cook::_ingest(PixVal &pix) {
  ...
  Val pixVal = pix.val;
  //debug
  //cout << "Checking ("<< x << "," << y << ") value =" << pixVal << " ";
  if(pixVal >= _threshold) {
    int objid;
    //debug
    //cout << " is above thrshld ";
...
void Cook::_flushFinalizable() {
  ...
  _obsPostProc opp;
  //cout << "enter flush 1" << endl; flush(cout);
  for_each(_finalizable.begin(), _finalizable.end(),_applyMap<_obsPostProc>(_obs, opp));
  // Write out.
//cout << "enter flush 2" << endl; flush(cout);
  _obsWrite obsWrite (_provider);
//cout << "humm" << endl; flush(cout);
  for_each(_finalizable.begin(), _finalizable.end(),_applyMap<_obsWrite>(_obs, obsWrite));
//cout << "enter flush 3" << endl; flush(cout);
  // Stop tracking
  for(OidIter i = _finalizable.begin(); i != _finalizable.end(); ++i) {
       _obs.erase(i->first);
       //cout << "Finalizing : " << i->first << endl;
  }
  // Reset Oid list.
  _finalizable = _fresh;
  _fresh.clear();
...
void Cook::_obsWrite::operator()(std::pair<const Oid,Observ>& p) {
  Observ& o = p.second;
  o.observId = p.first;
  //cout << "Fire on new observation" << endl; flush(cout);
