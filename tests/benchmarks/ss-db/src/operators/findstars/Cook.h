...
struct PixVal {
  ...
  scidb::Coordinates getCoord() {
      scidb::Coordinates pos(2);
    pos[0] = coord.second;
    pos[1] = coord.first;
    return pos;
  };
...
public:
  ...
  virtual bool nextArray() =0;
  virtual bool hasNextArray() =0;
...
public:
  ...
  Cook (PixelProvider &provider, int threshold) : _provider(provider), _threshold(threshold), _nextOid(1) {};
