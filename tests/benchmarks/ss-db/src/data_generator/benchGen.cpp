...
public:
    enum Mode { CSV, AFL, BINARY, ATTRIBUTE, POS_ONLY };
...
private:
    ...
    void _extract(std::ofstream& md, int size, int range, int t, int total,
                  std::string const& fileName, Mode outputMode, bool open, bool close);
...
/**
 * @param fileBase : for filename
 * @param n : number of chunks (default 1)
 * @param i : i must be < n, produce only the I^th chunks, for example total =
 * 10, chuncks=5, i =2 --> the chunks bock between 2 to 6 ( draw a gird)
 */
void Tiles::genData(std::string const& fileBase, int n, int i, bool roundRobin,
    ...
    if (roundRobin) {
        ...
        for (int t = 0; t < total; ++t) {
            ...
            if (oneFile) {
                _extract(md, size, range, t, total, fileBase, outputMode, next==1, next==n);
            } else {
                fname << fileBase << '_' << std::setfill('0') << std::setw(4) << t;
                _extract(md, size, range, t, total, fname.str(), outputMode, next==1, next==n);
            }
	    next++;
        }
        (outputMode == AFL)? md << "]\n": md;
        ...
        for (int t = low; t < high; ++t) {
            std::stringstream fname;
            if (oneFile) {
                _extract(md, size, range, t, total, fileBase, outputMode, (next==low)?true:false, (next==high-1)? true:false);
            } else {
                fname << fileBase << '_' << std::setfill('0') << std::setw(4) << t;
                _extract(md, size, range, t, total, fname.str(), outputMode, next==low, (next==high-1)? true:false);
            }
          next++;
        }
        (outputMode == AFL)? md << "]\n": md;
...
//     md << '{' << llcX << ',' << llcY << "}(\"" << config << '_' << std::setfill('0') << std::setw(4) << t << "\")";
    md << '(' << llcX << ',' << llcY << "," << t << ")";
    if (t < total-1) {
        md << ',';
    } 
    ...
    if (outputMode == BINARY) { // {{{ BINARY
        ...
        char rowbuf[ROWBUF_SIZE];

        static int const CHUNK_SIZE = 100;
        for (int yChunk = 0; yChunk <= (size / CHUNK_SIZE); ++yChunk) {
            int yChunkMin = yChunk * CHUNK_SIZE;
            int yChunkMax = yChunkMin + CHUNK_SIZE;
            if (yChunkMax > size) {
                yChunkMax = size;
            }
            for (int xChunk = 0; xChunk <= (size / CHUNK_SIZE); ++xChunk) {
                int xChunkMin = xChunk * CHUNK_SIZE;
                int xChunkMax = xChunkMin + CHUNK_SIZE;
                if (xChunkMax > size) {
                    xChunkMax = size;
                }
                int chunk = yChunk * 1000 + xChunk;

                for (int yOffset = yChunkMin; yOffset < yChunkMax; ++yOffset) {
                    for (int xOffset = xChunkMin; xOffset < xChunkMax;
                         ++xOffset) {
                        int x = xOffset + llcX;
                        int y = yOffset + llcY;
      ...
      static int const ROWBUF_SIZE = 14 * (11 + 1) + 1;
      // 11 numbers, each up to 10 digits plus sign plus separator,
      // 2 () comma, and one for terminating null.
      char rowbuf[ROWBUF_SIZE];

      //produce size*size array
      //TODO pcm : removing this block is correct
        ...
        for (int x = llcX; x < llcX + size; ++x) {
          ...
          int written = ::snprintf(
            ...
            "(%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)%c",
              p.pix, p.var, p.valid,
              p.sat, p.v0, p.v1, p.v2, 
              p.v3, p.v4, p.v5, p.v6, comma);
          if (written <= 0) {
            throw std::runtime_error("Failed to snprintf");
          }
          out.write(rowbuf, written);
    ...
    else if (outputMode == ATTRIBUTE) { // {{{ Attributes
...
void usage(char* argv0) {
    ...
    std::cerr << "\t-c CONFIG: tiny (default), small, normal, large, very-large" << std::endl;
    std::cerr << "\t-o: write alll data into one file" << std::endl;
    std::cerr << "\t-t: produce text (CSV) output (default=binary)" << std::endl;
    std::cerr << "\t-s: produce text, Scidb output format" << std::endl;
...
int main(int argc, char** argv) {
    ...
    int ch;
    while ((ch = ::getopt(argc, argv, "f:n:i:c:otsahpr")) != -1) {
        switch (ch) {
        case 'f':
            fbase = std::string(::optarg);
            break;
        case 'n':
            chunks = std::atoi(::optarg);
            break;
        case 'i':
    ...
    if (config == "tiny") {
        ...
        tiles.genData(fbase, chunks, index, roundRobin, 160, 3750, 10000, outputMode);
    }
    else if (config == "normal") {
        tiles.genData(fbase, chunks, index, roundRobin, 400, 7500, 31623, outputMode);
    }
    else if (config == "normal4") {
        tiles.genData(fbase, chunks, index, roundRobin, 4, 7500, 31623, outputMode);
    }
    else if (config == "large") {
        tiles.genData(fbase, chunks, index, roundRobin, 1000, 15000, 100000, outputMode);
    }
    else if (config == "very_large") {
        tiles.genData(fbase, chunks, index, roundRobin, 2500, 30000, 316228, outputMode);
    }
    else {
        usage(argv[0]);
    }
