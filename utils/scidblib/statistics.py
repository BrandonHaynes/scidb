...
#
# Modification notice from SciDB, Inc.
#
# We modified the file to make it compatible with Python 2.x.
...
# Modified by SciDB, Inc., to replace collections.Counter with counter.Counter.
from scidblib import counter

def scidb_fdiv(numerator, denominator):
    """SciDB modification: don't truncate division results to an integer (Python 2)."""
...
def _sum(data, start=0):
    ...
    if issubclass(T, Decimal):
        return scidb_fdiv(T(total.numerator), total.denominator)
    return T(total)
...
def _counts(data):
    ...
    #Modified by SciDB, Inc. to avoid using collections.Counter which did not work for Python 2.6.
    table = counter.Counter(iter(data)).most_common()
    if not table:
        return table
...
def median(data):
    ...
    if n%2 == 1:
        ...
        i = n//2
        return scidb_fdiv((data[i - 1] + data[i]), 2)
...
def median_grouped(data, interval=1):
    ...
    try:
        L = x - scidb_fdiv(interval, 2)  # The lower limit of the median interval.
    except TypeError:
    ...
    return L + interval*(scidb_fdiv(n, 2) - cf)/f
...
def _ss(data, c=None):
    ...
    # error may not.
    ss -= scidb_fdiv(_sum((x-c) for x in data)**2, len(data))
