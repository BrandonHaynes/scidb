...
namespace scidb
{
    ...
    size_t NullSuppression::compress(void* dst, const ConstChunk& chunk, size_t chunkSize) 
    {
        ...
        uint32_t maxBlock = blockLength * elementSize + 1; // maximum area a block can write 
        uint32_t written = 0;

        std::map<uint64_t, uint8_t> *encoder;
        ...
        {
            encoder = &_encode2Bits;
        }
        else if(elementSize == 8)
        {
            encoder = &_encode4Bits;
        }
        else
        {
        ...
        for(i = 0; i < blocks; ++i)
        {
            if(written + maxBlock >= chunkSize)
            {
                return chunkSize;
            }

            blockCode = writePtr; // where to write the codes for each value
            ++writePtr;
            ++written;
            for(j = 0; j < blockLength; ++j)
            {
                // record dictionary entry info
                bytesUsedPtr[j] = getBytes(readPtr, elementSize);

                // copy the remaining bytes and advance our ptrs
                memcpy(writePtr, readPtr, bytesUsedPtr[j]);

                writePtr += bytesUsedPtr[j];
                readPtr += elementSize;
                written += bytesUsedPtr[j];
            } // end for per block
            *blockCode = (*encoder)[bytesUsed];
        }// end for for whole blocks


        // check to see if we have space for the final, partial block
        if(endElements && (written + endElements*elementSize + 1 >= chunkSize))
        {
            return chunkSize;
        }
        ...
        for(i = 0; i < endElements; ++i)
        {
            ...
            // copy out the element for analysis and identify the # of bytes used
            bytesUsedPtr[i] = getBytes(readPtr, elementSize);
