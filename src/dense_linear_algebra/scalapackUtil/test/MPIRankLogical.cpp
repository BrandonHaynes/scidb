...
// REFACTOR
///
/// integer rounds down to the nearest integer
/// this version of division rounds up to the nearest integer
/// like all division operators, it causes an exception if the divisor is zero
///
...
class MPIRankLogical: public LogicalOperator
{
public:
    MPIRankLogical(const std::string& logicalName, const std::string& alias):
...
ArrayDesc MPIRankLogical::inferSchema(std::vector<ArrayDesc> schemas, boost::shared_ptr<Query> query)
{
    ...
    outDims[1] = DimensionDesc(d0.getBaseName() + "_2", 
                            ...
                            d1.getChunkInterval(), 
                            0);
    return ArrayDesc("SVD", atts, outDims);
}

REGISTER_LOGICAL_OPERATOR_FACTORY(MPIRankLogical, "mpirank");
